...
public class Poly {
	 ...
	 *             An operator error.
	 */
	public Poly(final String str) throws PolyException, OperatorException,
			Exception {
		...
		for (int i = 0; i < str.length(); i++) {
			...
			if (Character.isAlphabetic(c)) {
				this.setVariable(Character.toString(c));
			}

		}

		final String delimiters = "+-*()" + variable;
		...
		while (input.hasMoreTokens()) {
			...
			if (token.equals(variable)) {
				// Create a new polynomial with a single term.
	...
	public void iroots() {
		...
		if (this.degree == 0) {
			...
			final int coefficient = this.coeffs[i];
			final int upperValue = Math.abs(coefficient);
			final int lowerValue = -1 * Math.abs(coefficient);
	...
	public String toString() {
		...
		for (int exponent = this.degree; exponent >= 0; exponent--) {
			String str = "";
			int coefficient = this.coeffs[exponent];
			...
			if (coefficient != 0) {
				...
				final int c = Math.abs(coefficient);
	 ...
	 * 
	 * @param x
	 *            the string that is wanted to be used as a variable
	 */
